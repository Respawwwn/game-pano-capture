...
class GenericMouseHandler(IMouseHandler):
    ...
    def click(self, button: str = "left") -> bool:
        ...
        try:
            # Generate mouse down and up events
            pygame.event.post(pygame.event.Event(pygame.MOUSEBUTTONDOWN, {'button': button_id}))
            pygame.event.post(pygame.event.Event(pygame.MOUSEBUTTONUP, {'button': button_id}))
            pygame.event.pump()
    ...
    def scroll(self, dx: int = 0, dy: int = 0) -> bool:
        ...
        try:
            ...
            if dy != 0:
                pygame.event.post(pygame.event.Event(pygame.MOUSEWHEEL, {'y': dy, 'x': dx}))
                pygame.event.pump()
