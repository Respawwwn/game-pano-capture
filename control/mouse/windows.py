...
class WindowsMouseHandler(IMouseHandler):
    ...
    def __init__(self):
        ...
        if not WIN32_AVAILABLE and not CTYPES_AVAILABLE:
            self._logger.warning("Neither win32api nor ctypes available for mouse control")
            return
    ...
    def move_relative(self, dx: int, dy: int) -> bool:
        ...
        try:
            if WIN32_AVAILABLE:
                ...
                self._logger.debug(f"Moved mouse relatively by ({dx}, {dy}) using win32api")
                return True
            elif CTYPES_AVAILABLE:
                ...
                self._logger.debug(f"Moved mouse relatively by ({dx}, {dy}) using ctypes")
                return True
            else:
                return False
    ...
    def move_absolute(self, x: int, y: int) -> bool:
        ...
        try:
            if WIN32_AVAILABLE:
                ...
                self._logger.debug(f"Moved mouse to absolute position ({x}, {y}) using win32api")
                return True
            elif CTYPES_AVAILABLE:
                # Use ctypes for absolute positioning
                ctypes.windll.user32.SetCursorPos(x, y)
                self._logger.debug(f"Moved mouse to absolute position ({x}, {y}) using ctypes")
                return True
            else:
                return False
    ...
    def get_position(self) -> tuple[int, int]:
        ...
        try:
            if WIN32_AVAILABLE:
                ...
                self._logger.debug(f"Current mouse position: ({point.x}, {point.y}) via ctypes")
    ...
    def scroll(self, dx: int = 0, dy: int = 0) -> bool:
        ...
        try:
            ...
            if WIN32_AVAILABLE:
                # Vertical scroll
                if dy != 0:
                    win32api.mouse_event(win32con.MOUSEEVENTF_WHEEL, 0, 0, wheel_delta_y, 0)
                # Horizontal scroll
                if dx != 0:
                    win32api.mouse_event(win32con.MOUSEEVENTF_HWHEEL, 0, 0, wheel_delta_x, 0)
                self._logger.debug(f"Scrolled mouse wheel (dx={dx}, dy={dy}) using win32api")
                return True
            elif CTYPES_AVAILABLE:
                # Vertical scroll
                if dy != 0:
                    ctypes.windll.user32.mouse_event(0x0800, 0, 0, wheel_delta_y, 0)  # MOUSEEVENTF_WHEEL
                # Horizontal scroll
                if dx != 0:
                    ctypes.windll.user32.mouse_event(0x01000, 0, 0, wheel_delta_x, 0)  # MOUSEEVENTF_HWHEEL
                self._logger.debug(f"Scrolled mouse wheel (dx={dx}, dy={dy}) using ctypes")
                return True
            else:
                return False
