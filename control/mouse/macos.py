...
try:
    import Quartz
    from Quartz import CGEventCreateMouseEvent, CGEventPost, CGEventSourceCreate
    from Quartz import kCGEventMouseMoved, kCGEventLeftMouseDown, kCGEventLeftMouseUp
    from Quartz import kCGEventRightMouseDown, kCGEventRightMouseUp, kCGEventOtherMouseDown
    from Quartz import kCGEventOtherMouseUp, kCGEventScrollWheel, kCGHIDEventTap
    from Quartz import kCGEventSourceStateHIDSystemState, CGEventCreateScrollWheelEvent
    from Quartz import CGDisplayBounds, CGMainDisplayID, CGEventGetLocation
...
class MacOSMouseHandler(IMouseHandler):
    ...
    def move_relative(self, dx: int, dy: int) -> bool:
        ...
        try:
            ...
            self._logger.debug(f"Moved mouse relatively by ({dx}, {dy}) to ({new_x}, {new_y})")
            return True
