...
class WindowsGamepadHandler(IGamepadHandler):
    ...
    def initialize(self) -> bool:
        ...
        if not VGAMEPAD_AVAILABLE:
            self._logger.error(
                "vgamepad not available - gamepad support requires vgamepad package"
            )
            return False
    ...
    def move_stick(
        ...
        try:
            ...
            if stick.lower() == "left":
                self._virtual_gamepad.left_joystick_float(
                    x_value_float=x_value, y_value_float=y_value
                )
            elif stick.lower() == "right":
                self._virtual_gamepad.right_joystick_float(
                    x_value_float=x_value, y_value_float=y_value
                )
            else:
            ...
            self._logger.debug(
                f"Moved {stick} stick to ({x_value:.2f}, {y_value:.2f}) for {duration}s"
            )
            return True
    ...
    def press_trigger(self, trigger: str, value: float, duration: float = 0.1) -> bool:
        ...
        try:
            ...
            self._logger.debug(
                f"Pressed {trigger} trigger to {value:.2f} for {duration}s"
            )
            return True
